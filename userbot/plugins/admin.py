...
async def nothanos(unbon):
    ...
    try:
        ...
        await unbon.edit("```Bhuro Pa6o avi skse```")
...
async def spider(spdr):
    ...
    await spdr.edit("`Mungo mar!`")
    if mute(spdr.chat_id, user.id) is False:
        return await spdr.edit('`Error! Lagbhag bhuro pelethi j boli ske 6.`')
    else:
        try:
            await spdr.client(
            ...
            if reason:
                await spdr.edit(f"`Bhuro mungo thay gyo`\nKaran ke: {reason}")
            else:
                await spdr.edit("`Bhuro mungo thay gyo !!`")
...
async def pin(msg):
    ...
    await msg.edit("`Bhai ke ane hu pin no kru evu no bane\nPinned Successfully!`")
